...
async function renderDocumentHtml(documentHtml, pageContext, renderSrc, onErrorWhileStreaming, injectFilter) {
...
async function renderTemplate(templateContent, renderSrc, pageContext) {
    ...
    const setStream = (stream) => {
        (0, utils_1.assertUsage)(!htmlStream, `Injecting two streams in \`escapeInject\` template tag of render() hook defined by ${renderSrc}. Inject only one stream instead.`);
        htmlStream = stream;
    };
    ...
    for (let i = 0; i < templateVariables.length; i++) {
        ...
        if (templateVar === undefined) {
            (0, utils_1.assertWarning)(false, getErrMsg(`\`${templateVar}\``, ''), { showStackTrace: false, onlyOnce: false });
            continue;
        }
        {
