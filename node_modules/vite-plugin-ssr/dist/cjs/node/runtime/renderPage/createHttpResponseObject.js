...
async function createHttpResponseObject(htmlRender, renderSrc, pageContext) {
    if (htmlRender === null) {
        return null;
    }
    (0, utils_1.assert)(renderSrc || typeof htmlRender === 'string');
    let statusCode;
    {
    ...
    function errMsg(method, fixMsg) {
        ...
        (0, utils_1.assert)(renderSrc);
        return [
            `\`pageContext.httpResponse.${method}\` can't be used because your \`render()\` hook defined by ${renderSrc} provides ${htmlRenderName}`,
            fixMsg,
            streamDocs
        ]
            .filter(Boolean)
...
function removeFileExtentionAndHash(assetUrl) {
    (0, utils_1.assert)(assetUrl.startsWith('/'));
