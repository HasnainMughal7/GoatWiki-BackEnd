...
const retrieveAssetsDev_1 = require("./getPageAssets/retrieveAssetsDev");
const retrieveAssetsProd_1 = require("./getPageAssets/retrieveAssetsProd");
const path_1 = __importDefault(require("path"));
const helpers_1 = require("../helpers");
const getManifestEntry_1 = require("./getPageAssets/getManifestEntry");
const sortPageAssetsForEarlyHintsHeader_1 = require("./getPageAssets/sortPageAssetsForEarlyHintsHeader");
const globalContext_1 = require("../globalContext");
const assertClientEntryId_1 = require("./getPageAssets/assertClientEntryId");
async function getPageAssets(pageContext, clientDependencies, clientEntries) {
    ...
    if (isDev) {
        ...
        clientEntriesSrc = clientEntries.map((clientEntry) => resolveClientEntriesDev(clientEntry, viteDevServer, configVps));
        assetUrls = await (0, retrieveAssetsDev_1.retrieveAssetsDev)(clientDependencies, viteDevServer);
    }
    else {
        ...
        assetUrls = (0, retrieveAssetsProd_1.retrieveAssetsProd)(clientDependencies, clientManifest, pageContext._includeAssetsImportedByServer, manifestKeyMap);
    }
    let pageAssets = [];
    (0, utils_1.unique)([...clientEntriesSrc, ...assetUrls]).forEach((src) => {
        const { mediaType = null, assetType = null } = (0, helpers_1.inferMediaType)(src) || {};
    ...
    pageAssets.forEach(({ src }) => {
        (0, utils_1.assert)(1 === pageAssets.filter((p) => p.src === src).length);
    });
    pageAssets = pageAssets.map((pageAsset) => {
        const baseServerAssets = pageContext._baseAssets || pageContext._baseServer;
        pageAsset.src = (0, utils_1.prependBase)((0, utils_1.normalizePath)(pageAsset.src), baseServerAssets);
        return pageAsset;
    });
    (0, sortPageAssetsForEarlyHintsHeader_1.sortPageAssetsForEarlyHintsHeader)(pageAssets);
    return pageAssets;
}
exports.getPageAssets = getPageAssets;
function resolveClientEntriesDev(clientEntry, viteDevServer, configVps) {
    (0, assertClientEntryId_1.assertClientEntryId)(clientEntry);
    ...
    if (path_1.default.posix.isAbsolute(clientEntry)) {
        ...
        try {
            ...
            filePath = (0, utils_1.toPosixPath)(res(clientEntry.replace('@@vite-plugin-ssr/dist/esm/client/', '../../../client/').replace('.js', '.ts')));
        }
        catch {
            // For users
            // [RELATIVE_PATH_FROM_DIST] Current file: node_modules/vite-plugin-ssr/dist/cjs/node/runtime/html/injectAssets.js
            filePath = (0, utils_1.toPosixPath)(res(clientEntry.replace('@@vite-plugin-ssr/dist/esm/client/', '../../../../../dist/esm/client/')));
        }
    }
    else if ((0, utils_1.isNpmPackageModule)(clientEntry)) {
        ...
        (0, utils_1.assert)(extensionPageFile, clientEntry);
...
function resolveClientEntriesProd(clientEntry, clientManifest, manifestKeyMap) {
    const { manifestEntry } = (0, getManifestEntry_1.getManifestEntry)(clientEntry, clientManifest, manifestKeyMap);
    (0, utils_1.assert)(manifestEntry.isEntry || manifestEntry.isDynamicEntry || clientEntry.endsWith('.css'), { clientEntry });
