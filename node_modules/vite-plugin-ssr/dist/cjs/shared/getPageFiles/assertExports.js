...
function assertExportValues(pageFile) {
    enforceTrue.forEach((exportName) => {
        ...
        (0, utils_1.assertUsage)(pageFile.fileExports[exportName] !== false, `${pageFile.filePath} has \`export { ${exportName} }\` with the value \`false\` which is forbidden: remove \`export { ${exportName} }\` instead. (${explainer})`);
...
function assertDefaultExports(defaultExportName, filePath) {
    (0, utils_1.assertUsage)(!forbiddenDefaultExports.includes(defaultExportName), `${filePath} has \`export default { ${defaultExportName} }\` which is forbidden, use \`export { ${defaultExportName} }\` instead.`);
}
