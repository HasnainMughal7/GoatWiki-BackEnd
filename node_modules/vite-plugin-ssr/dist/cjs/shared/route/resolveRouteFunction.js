...
async function resolveRouteFunction(routeFunction, allowAsync, pageContext, pageRouteFilePath) {
    ...
    (0, utils_1.assertUsage)(!(0, utils_1.isPromise)(result) || allowAsync, `The Route Function ${pageRouteFilePath} returned a promise; async route functions are opt-in, see https://vite-plugin-ssr.com/route-function#async`);
    result = await result;
    if (result === false) {
        return null;
    }
    ...
    if ('match' in result) {
        const { match } = result;
        (0, utils_1.assertUsage)(typeof match === 'boolean', `The \`match\` value returned by the Route Function ${pageRouteFilePath} should be a boolean.`);
        if (!match) {
            return null;
        }
    }
    ...
    (0, utils_1.assertUsage)(!('pageContext' in result), 'Providing `pageContext` in Route Functions is forbidden, see https://vite-plugin-ssr.com/route-function#cannot-provide-pagecontext');
