...
function assertPageContextProvidedByUser(pageContextProvidedByUser, { hook, errorMessagePrefix, canBePromise }) {
    ...
    if (canBePromise && !isObject(pageContextProvidedByUser)) {
        assertUsage(isCallable(pageContextProvidedByUser) || isPromise(pageContextProvidedByUser), `${errorMessagePrefix} should be an object, or an async function https://vite-plugin-ssr.com/stream#initial-data-after-stream-end`);
        return;
    }
    ...
    assertWarning(!('_pageId' in pageContextProvidedByUser), 'You are using `onBeforeRoute()` to override vite-plugin-ssr routing. This is experimental: make sure to contact a vite-plugin-ssr maintainer before using this.', { showStackTrace: false, onlyOnce: true });
}
